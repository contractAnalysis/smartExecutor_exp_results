...
def compare_two_tools(tool1:str, base_dir1:str,tool2:str,base_dir2:str,parent_dir:str):
    ...
    columns_needed=['solidity','solc','contract','time','num_states','cov_2','vulnerability','function_cov']
    ...
    df_tool1['num_states']=df_tool1['num_states'].map(lambda x:int(x))
    df_tool1['cov_2']=df_tool1['cov_2'].map(lambda x: float(str(x).strip('%')))

    df_tool1.columns=['solidity','solc','contract','time_x','num_states_x','cov_2_x','vulnerability_x','function_cov_x']
    ...
    df_tool2['num_states']=df_tool2['num_states'].map(lambda x:int(x))
    df_tool2['cov_2']=df_tool2['cov_2'].map(lambda x: float(str(x).strip('%')))
    df_tool2.columns=['solidity','solc','contract','time_y','num_states_y','cov_2_y','vulnerability_y','function_cov_y']
...
def get_general_data_statistics(df_data:pd.DataFrame):
    ...
    total_state_1=df_data['num_states_x'].sum()
    total_state_2=df_data['num_states_y'].sum()
    ...
    general_data=[total_contracts,total_time_1,total_state_1,total_bugs_1,ave_time_1,ave_cov_1,total_ftn_high_cov]+\
    [total_time_2,total_state_2,total_bugs_2,ave_time_2,ave_cov_2]
    return general_data
...
results=[['results','total_contracts','total_time_1','total_num_states_1','total_bugs_1','ave_time_1','ave_cov_1','total_ftn_high_cov']+\
         ['total_time_2','total_num_states_2','total_bugs_2','ave_time_2','ave_cov_2']
         ] 
ftn_results={}
